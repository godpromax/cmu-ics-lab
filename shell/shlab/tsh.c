...
void eval(char *cmdline) 
{
    ...
    int bg;                     /* 这个作业应该在后台进行？ */
    pid_t pid;                  /* 进程id*/
    strcpy(buf,cmdline);        //将命令行中输入的字符串复制给buf
    bg = parseline(buf,argv);   //判断前后台进程
    if(argv[0] == NULL)
        return;                 /* 忽略空命令 */
 
    sigset_t mask_all,mask_one,prev_one;//建立三个信号的集合，分别表示所有信号和当前信号和父进程信号
    if(!builtin_cmd(argv)){     //判断是否是内置指令，不是内置命令就fork一个子进程执行程序
        ...
        sigprocmask(SIG_BLOCK,&mask_one,&prev_one);         /* 添加mask_one中的信号到prev_one信号集合（blocked位向量），从而父进程保持SIGCHLD的阻塞*/
        if((pid = fork()) == 0){    /* 子程序运行用户作业 */
 
            sigprocmask(SIG_SETMASK,&prev_one,NULL); /* 因为子进程继承了它们父进程的被阻塞集合，所以在调用execve之前，先将信号阻塞清空，必须
       ...
       if(setpgid(0,0) < 0){               /* 把子进程放到一个新进程组中，该进程组ID与子进程的PID相同。这将确保前台进程组中只有一个进程，即shell进程。*/
        ...
        if(execve(argv[0],argv,environ) < 0){  //这里就是执行命令行argv[0]是开始给定的执行路径，argv是后面包含的参数，environ是环境变量
       ...
       //第一个错误处理，直接在这里进行提示信息输出
        ...
        sigprocmask(SIG_SETMASK,&prev_one,NULL);  /* 解除子进程对SIGCHLD的阻塞 解除阻塞之后父进程就能收到sigchld信号对子进程进行回收*/
...
void do_bgfg(char **argv) 
{
   ...
   //在这里判断并输出错误信息：fg: argument must be a PID or %%jobid\n
        jid = atoi(argv[1]+1);//atoi函数：把字符串argv[1]+1后转换为int。
...
//第五个错误就是通过jid找到的job==null，因此“NO such job”
    ...
    if(pid > 0){
        if(!strcmp(argv[0],"bg")){          /* bg内置指令 */
            ...
            job->state = BG;                /* 更改状态 */
            kill(-pid,SIGCONT);              /* kill函数时传递信号，传递SIGCONT信号给进程组中的所有进程 */
 ...
 */
void sigchld_handler(int sig) //子进程给父进程的回收信号，避免僵尸进程
{
    int olderrno = errno;
