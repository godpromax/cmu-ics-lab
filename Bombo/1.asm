...
08048a1d <main>:
 ...
 8048b13:	e8 ea 00 00 00       	call   8048c02 <phase_3>
 ...
 8048b31:	e8 ca 01 00 00       	call   8048d00 <phase_4>
 ...
 8048b4f:	e8 0d 02 00 00       	call   8048d61 <phase_5>
 ...
 8048b6d:	e8 73 02 00 00       	call   8048de5 <phase_6>
...
08048b90 <phase_1>:
 ...
 8048b9a:	08 
 8048b9b:	8b 44 24 20          	mov    0x20(%esp),%eax              //输入的字符串
...
08048bb4 <phase_2>:
 8048bb4:	56                   	push   %esi
 8048bb5:	53                   	push   %ebx
 8048bb6:	83 ec 34             	sub    $0x34,%esp                    //把esp的值减去0x34在保存到esp中
 8048bb9:	8d 44 24 18          	lea    0x18(%esp),%eax               //此时eax的地址为esp+18
 8048bbd:	89 44 24 04          	mov    %eax,0x4(%esp)                //将eax的值保存到esp+4中，函数中的第一个变量
 8048bc1:	8b 44 24 40          	mov    0x40(%esp),%eax               //将esp+0x40的值保存到eax中，输入的变量
 8048bc5:	89 04 24             	mov    %eax,(%esp)                   //esp=eax，为函数运行做准备
 8048bc8:	e8 7f 05 00 00       	call   804914c <read_six_numbers>
 8048bcd:	83 7c 24 18 01       	cmpl   $0x1,0x18(%esp)               //比较1和esp+18地址的值
 8048bd2:	74 1e                	je     8048bf2 <phase_2+0x3e>        //为了不炸esp+18的值只能是1
 8048bd4:	e8 4c 05 00 00       	call   8049125 <explode_bomb>        //炸弹1
 8048bd9:	eb 17                	jmp    8048bf2 <phase_2+0x3e>        //直接跳
 8048bdb:	8b 43 fc             	mov    -0x4(%ebx),%eax               //把ebx-4的值存到eax，输入变量
 8048bde:	01 c0                	add    %eax,%eax                     //2*eax
 8048be0:	39 03                	cmp    %eax,(%ebx)                   //比较eax和ebx
 8048be2:	74 05                	je     8048be9 <phase_2+0x35>        //eax和ebx不相等就炸
 8048be4:	e8 3c 05 00 00       	call   8049125 <explode_bomb>        //炸弹2
 8048be9:	83 c3 04             	add    $0x4,%ebx                     //ebx的值加上4
 8048bec:	39 f3                	cmp    %esi,%ebx                     //比较esi和ebx
 8048bee:	75 eb                	jne    8048bdb <phase_2+0x27>        //不相等跳
 8048bf0:	eb 0a                	jmp    8048bfc <phase_2+0x48>        //直接跳
 8048bf2:	8d 5c 24 1c          	lea    0x1c(%esp),%ebx               //把esp+0x1c的地址存到ebx中
 8048bf6:	8d 74 24 30          	lea    0x30(%esp),%esi               //吧esp+0x30的地址存到esi中
 8048bfa:	eb df                	jmp    8048bdb <phase_2+0x27>        //直接跳
...
08048c02 <phase_3>:
 8048c02:	83 ec 2c             	sub    $0x2c,%esp                      //esp-0x2c
 8048c05:	8d 44 24 1c          	lea    0x1c(%esp),%eax                 //此时eax存的值为esp+0x1c的地址
 8048c09:	89 44 24 0c          	mov    %eax,0xc(%esp)                  //esp+0xc存eax的值，就是上面esp+0x1c的地址
 8048c0d:	8d 44 24 18          	lea    0x18(%esp),%eax                 //此时eax存的值为esp+0x18的地址
 8048c11:	89 44 24 08          	mov    %eax,0x8(%esp)                  //esp+8中存eax的值，就是上面esp+0x18的地址
 8048c15:	c7 44 24 04 c7 a2 04 	movl   $0x804a2c7,0x4(%esp)            //这时候esp+4从一个立即数中读取了一个值存进去
 8048c1c:	08 
 8048c1d:	8b 44 24 30          	mov    0x30(%esp),%eax                 //eax=esp+0x30，之前输入的变量
 8048c21:	89 04 24             	mov    %eax,(%esp)                     //esp=eax=esp+0x30
 8048c24:	e8 37 fc ff ff       	call   8048860 <__isoc99_sscanf@plt>   //三个参数
 8048c29:	83 f8 01             	cmp    $0x1,%eax                       //比较1和eax，eax是sscanf函数的返回值
 8048c2c:	7f 05                	jg     8048c33 <phase_3+0x31>          //如果eax>1就跳转，所以eax大于1才能不炸，表示sscanf返回成功，eax返回的是成功输入的变量个数
 8048c2e:	e8 f2 04 00 00       	call   8049125 <explode_bomb>          //炸弹1
 8048c33:	83 7c 24 18 07       	cmpl   $0x7,0x18(%esp)                 //比较7和esp+0x18
 8048c38:	77 5c                	ja     8048c96 <phase_3+0x94>          //无符号＞比较，esp+18的值＞7就救炸，所以的esp+18≤7
 8048c3a:	8b 44 24 18          	mov    0x18(%esp),%eax                 //eax=esp+0x18
 8048c3e:	ff 24 85 60 a1 04 08 	jmp    *0x804a160(,%eax,4)             //跳到eax*4+0x804a160的地址，即4*（esp+0x18）拿寄存器存的值算
 8048c45:	b8 00 00 00 00       	mov    $0x0,%eax                       //eax清零
 8048c4a:	eb 05                	jmp    8048c51 <phase_3+0x4f>          //
 8048c4c:	b8 92 00 00 00       	mov    $0x92,%eax                      //eax=立即数
 8048c51:	2d 30 02 00 00       	sub    $0x230,%eax                     //eax-0x230
 8048c56:	eb 05                	jmp    8048c5d <phase_3+0x5b>          //
 8048c58:	b8 00 00 00 00       	mov    $0x0,%eax                       //eax清零
 8048c5d:	05 73 03 00 00       	add    $0x373,%eax                     //eax+0x373
 8048c62:	eb 05                	jmp    8048c69 <phase_3+0x67>          //
 8048c64:	b8 00 00 00 00       	mov    $0x0,%eax                       //eax清零
 8048c69:	83 e8 72             	sub    $0x72,%eax                      //eax-0x72
 8048c6c:	eb 05                	jmp    8048c73 <phase_3+0x71>          //
 8048c6e:	b8 00 00 00 00       	mov    $0x0,%eax                       //eax清零
 8048c73:	83 c0 72             	add    $0x72,%eax                      //eax+0x72
 8048c76:	eb 05                	jmp    8048c7d <phase_3+0x7b>          //
 8048c78:	b8 00 00 00 00       	mov    $0x0,%eax                       //eax清零
 8048c7d:	83 e8 72             	sub    $0x72,%eax                      //eax-0x72
 8048c80:	eb 05                	jmp    8048c87 <phase_3+0x85>          //
 8048c82:	b8 00 00 00 00       	mov    $0x0,%eax                       //eax清零
 8048c87:	83 c0 72             	add    $0x72,%eax                      //eax+0x72
 8048c8a:	eb 05                	jmp    8048c91 <phase_3+0x8f>          //
 8048c8c:	b8 00 00 00 00       	mov    $0x0,%eax                       //eax清零
 8048c91:	83 e8 72             	sub    $0x72,%eax                      //eax-0x72
 8048c94:	eb 0a                	jmp    8048ca0 <phase_3+0x9e>          //
 8048c96:	e8 8a 04 00 00       	call   8049125 <explode_bomb>          //
 8048c9b:	b8 00 00 00 00       	mov    $0x0,%eax                       //
 8048ca0:	83 7c 24 18 05       	cmpl   $0x5,0x18(%esp)                 //比较5和esp+18的值
 8048ca5:	7f 06                	jg     8048cad <phase_3+0xab>          //esp+18＞5就跳转，就炸
 8048ca7:	3b 44 24 1c          	cmp    0x1c(%esp),%eax                 //比较eax和esp+0x1c的值
 8048cab:	74 05                	je     8048cb2 <phase_3+0xb0>          //相等就通过，不炸
 8048cad:	e8 73 04 00 00       	call   8049125 <explode_bomb>          //炸弹3
 8048cb2:	83 c4 2c             	add    $0x2c,%esp                      //
 8048cb5:	c3                   	ret    
...
08048cb6 <func4>:
 ...
 8048cb7:	56                   	push   %esi                      //
 8048cb8:	53                   	push   %ebx                      //
 8048cb9:	83 ec 10             	sub    $0x10,%esp                //esp-0x10
 8048cbc:	8b 5c 24 20          	mov    0x20(%esp),%ebx           //第一个参数ebx
 8048cc0:	8b 74 24 24          	mov    0x24(%esp),%esi           //第二个参数esi
 8048cc4:	85 db                	test   %ebx,%ebx                 //
 8048cc6:	7e 2c                	jle    8048cf4 <func4+0x3e>      //输入的参数为0就结束递归
 8048cc8:	89 f0                	mov    %esi,%eax                 //递归中的esi返回值
 8048cca:	83 fb 01             	cmp    $0x1,%ebx                 //比较ebx和1
 8048ccd:	74 2a                	je     8048cf9 <func4+0x43>      //ebx=1跳转，如果参数≤1就结束递归
 8048ccf:	89 74 24 04          	mov    %esi,0x4(%esp)            //esp+4=esi
 8048cd3:	8d 43 ff             	lea    -0x1(%ebx),%eax           //eax存ebx-1的地址
 8048cd6:	89 04 24             	mov    %eax,(%esp)               //esp=eax=ebx-1，ebx-1为下面递归做准备
 8048cd9:	e8 d8 ff ff ff       	call   8048cb6 <func4>           //递归
 8048cde:	8d 3c 30             	lea    (%eax,%esi,1),%edi        //edi存eax+esi的地址
 8048ce1:	89 74 24 04          	mov    %esi,0x4(%esp)            //esp+4=esi
 8048ce5:	83 eb 02             	sub    $0x2,%ebx                 //ebx-2
 8048ce8:	89 1c 24             	mov    %ebx,(%esp)               //esp=ebx
 8048ceb:	e8 c6 ff ff ff       	call   8048cb6 <func4>           //递归，两个递归
 8048cf0:	01 f8                	add    %edi,%eax                 //eax=eax+edi
 8048cf2:	eb 05                	jmp    8048cf9 <func4+0x43>      //结束递归
 8048cf4:	b8 00 00 00 00       	mov    $0x0,%eax                 //eax清零
 8048cf9:	83 c4 10             	add    $0x10,%esp                //esp+0x10
 8048cfc:	5b                   	pop    %ebx                      //
 8048cfd:	5e                   	pop    %esi                      //
 8048cfe:	5f                   	pop    %edi                      //
 8048cff:	c3                   	ret    
...
08048d00 <phase_4>:
 8048d00:	83 ec 2c             	sub    $0x2c,%esp                        //
 8048d03:	8d 44 24 18          	lea    0x18(%esp),%eax                   //eax存esp+18的地址
 8048d07:	89 44 24 0c          	mov    %eax,0xc(%esp)                    //esp+c=eax=esp+18，为
 8048d0b:	8d 44 24 1c          	lea    0x1c(%esp),%eax                   //
 8048d0f:	89 44 24 08          	mov    %eax,0x8(%esp)                    //
 8048d13:	c7 44 24 04 c7 a2 04 	movl   $0x804a2c7,0x4(%esp)              //
 8048d1a:	08 
 8048d1b:	8b 44 24 30          	mov    0x30(%esp),%eax                   //
 8048d1f:	89 04 24             	mov    %eax,(%esp)                       //
 8048d22:	e8 39 fb ff ff       	call   8048860 <__isoc99_sscanf@plt>     //还是三个参数
 8048d27:	83 f8 02             	cmp    $0x2,%eax                         //查看是否两个参数
 8048d2a:	75 0c                	jne    8048d38 <phase_4+0x38>            //输入的参数不是两个就炸
 8048d2c:	8b 44 24 18          	mov    0x18(%esp),%eax                   //输入的第一个变量
 8048d30:	83 e8 02             	sub    $0x2,%eax                         //eax-2
 8048d33:	83 f8 02             	cmp    $0x2,%eax                         //eax和2比较
 8048d36:	76 05                	jbe    8048d3d <phase_4+0x3d>            //eax≤2就跳转，所以eax小于等于2，否则炸，相当于第二个变量要2到4之间
 8048d38:	e8 e8 03 00 00       	call   8049125 <explode_bomb>            //
 8048d3d:	8b 44 24 18          	mov    0x18(%esp),%eax                   //
 8048d41:	89 44 24 04          	mov    %eax,0x4(%esp)                    //esp+0x4=esp+0x18
 8048d45:	c7 04 24 07 00 00 00 	movl   $0x7,(%esp)                       //esp=7
 8048d4c:	e8 65 ff ff ff       	call   8048cb6 <func4>                   //递归
 8048d51:	3b 44 24 1c          	cmp    0x1c(%esp),%eax                   //比较递归后的结果是否符合
 8048d55:	74 05                	je     8048d5c <phase_4+0x5c>            //不符合就炸
 8048d57:	e8 c9 03 00 00       	call   8049125 <explode_bomb>            //
 8048d5c:	83 c4 2c             	add    $0x2c,%esp                        //归还栈空间
 8048d5f:	90                   	nop
 8048d60:	c3                   	ret    

08048d61 <phase_5>:
 8048d61:	53                   	push   %ebx
 8048d62:	83 ec 28             	sub    $0x28,%esp
 8048d65:	8b 5c 24 30          	mov    0x30(%esp),%ebx
 8048d69:	65 a1 14 00 00 00    	mov    %gs:0x14,%eax
 8048d6f:	89 44 24 1c          	mov    %eax,0x1c(%esp)
 8048d73:	31 c0                	xor    %eax,%eax
 8048d75:	89 1c 24             	mov    %ebx,(%esp)
 8048d78:	e8 7e 02 00 00       	call   8048ffb <string_length>
 8048d7d:	83 f8 06             	cmp    $0x6,%eax                       //说明要输入一个6个大小的字符串
 8048d80:	74 45                	je     8048dc7 <phase_5+0x66>
 8048d82:	e8 9e 03 00 00       	call   8049125 <explode_bomb>
 8048d87:	eb 3e                	jmp    8048dc7 <phase_5+0x66>
 8048d89:	0f b6 14 03          	movzbl (%ebx,%eax,1),%edx              //edx=ebx+eax；eax循环一次加1，所以按顺序将字符串保存
 8048d8d:	83 e2 0f             	and    $0xf,%edx                       //按位进行与运算，把低四位保存，高位随意，六个字符串一个一个来
 8048d90:	0f b6 92 80 a1 04 08 	movzbl 0x804a180(%edx),%edx            //神秘长字符串的位置，为后续字符串匹配做准备，且是隐藏关位置
 8048d97:	88 54 04 15          	mov    %dl,0x15(%esp,%eax,1)           //dl是4位的寄存器，用来装低四位的东西
 8048d9b:	83 c0 01             	add    $0x1,%eax                       //eax加1
 8048d9e:	83 f8 06             	cmp    $0x6,%eax                       //eax与6比
 8048da1:	75 e6                	jne    8048d89 <phase_5+0x28>          //eax≠6就跳回去继续保存字符的低四位
 8048da3:	c6 44 24 1b 00       	movb   $0x0,0x1b(%esp)
 8048da8:	c7 44 24 04 59 a1 04 	movl   $0x804a159,0x4(%esp)            //0x804a159这个地址存的是“flames”
 8048daf:	08 
 8048db0:	8d 44 24 15          	lea    0x15(%esp),%eax
 8048db4:	89 04 24             	mov    %eax,(%esp)                     //esp=esp+0x15
 8048db7:	e8 5e 02 00 00       	call   804901a <strings_not_equal>
 8048dbc:	85 c0                	test   %eax,%eax
 8048dbe:	74 0e                	je     8048dce <phase_5+0x6d>          //返回值eax如果是0就不炸，说明字符串匹配
 8048dc0:	e8 60 03 00 00       	call   8049125 <explode_bomb>
 8048dc5:	eb 07                	jmp    8048dce <phase_5+0x6d>
 8048dc7:	b8 00 00 00 00       	mov    $0x0,%eax                       //eax清零
 8048dcc:	eb bb                	jmp    8048d89 <phase_5+0x28>
 8048dce:	8b 44 24 1c          	mov    0x1c(%esp),%eax
 8048dd2:	65 33 05 14 00 00 00 	xor    %gs:0x14,%eax
 8048dd9:	74 05                	je     8048de0 <phase_5+0x7f>
 8048ddb:	e8 e0 f9 ff ff       	call   80487c0 <__stack_chk_fail@plt>
 8048de0:	83 c4 28             	add    $0x28,%esp
 8048de3:	5b                   	pop    %ebx
 8048de4:	c3                   	ret    
...
08048de5 <phase_6>:
 8048de5:	56                   	push   %esi
 8048de6:	53                   	push   %ebx
 8048de7:	83 ec 44             	sub    $0x44,%esp
 8048dea:	8d 44 24 10          	lea    0x10(%esp),%eax
 8048dee:	89 44 24 04          	mov    %eax,0x4(%esp)
 8048df2:	8b 44 24 50          	mov    0x50(%esp),%eax
 8048df6:	89 04 24             	mov    %eax,(%esp)
 8048df9:	e8 4e 03 00 00       	call   804914c <read_six_numbers>
 8048dfe:	be 00 00 00 00       	mov    $0x0,%esi
 8048e03:	8b 44 b4 10          	mov    0x10(%esp,%esi,4),%eax         //一个个取数字
 8048e07:	83 e8 01             	sub    $0x1,%eax                      //
 8048e0a:	83 f8 05             	cmp    $0x5,%eax                      //第一个比较的地方，看看是不是减去1之后是不是≤5，是就成功，不是就炸，数字不能大于6
 8048e0d:	76 05                	jbe    8048e14 <phase_6+0x2f>
 8048e0f:	e8 11 03 00 00       	call   8049125 <explode_bomb>
 8048e14:	83 c6 01             	add    $0x1,%esi                      //esi+1说明上面mov的时候偏移多四个所以是下一个数字
 8048e17:	83 fe 06             	cmp    $0x6,%esi                      //看这个时候取了多少个数字，这里是跳出循环的关键
 8048e1a:	75 07                	jne    8048e23 <phase_6+0x3e>         //没取到6个就跳
 8048e1c:	bb 00 00 00 00       	mov    $0x0,%ebx
 8048e21:	eb 38                	jmp    8048e5b <phase_6+0x76>
 8048e23:	89 f3                	mov    %esi,%ebx
 8048e25:	8b 44 9c 10          	mov    0x10(%esp,%ebx,4),%eax         //ebx=esi，继续取数字
 8048e29:	39 44 b4 0c          	cmp    %eax,0xc(%esp,%esi,4)          //
 8048e2d:	75 05                	jne    8048e34 <phase_6+0x4f>         //说明前两个数字不能相等，不然炸
 8048e2f:	e8 f1 02 00 00       	call   8049125 <explode_bomb>
 8048e34:	83 c3 01             	add    $0x1,%ebx                      
 8048e37:	83 fb 05             	cmp    $0x5,%ebx                      
 8048e3a:	7e e9                	jle    8048e25 <phase_6+0x40>         //一个循环，说明两两个数之间不能相等
 8048e3c:	eb c5                	jmp    8048e03 <phase_6+0x1e>         //jmp回去取数字
 8048e3e:	8b 52 08             	mov    0x8(%edx),%edx                 
 8048e41:	83 c0 01             	add    $0x1,%eax
 8048e44:	39 c8                	cmp    %ecx,%eax
 8048e46:	75 f6                	jne    8048e3e <phase_6+0x59>
 8048e48:	eb 05                	jmp    8048e4f <phase_6+0x6a>
 8048e4a:	ba 3c c1 04 08       	mov    $0x804c13c,%edx                //链表首地址
 8048e4f:	89 54 b4 28          	mov    %edx,0x28(%esp,%esi,4)
 8048e53:	83 c3 01             	add    $0x1,%ebx
 8048e56:	83 fb 06             	cmp    $0x6,%ebx
 8048e59:	74 17                	je     8048e72 <phase_6+0x8d>
 8048e5b:	89 de                	mov    %ebx,%esi                       
 8048e5d:	8b 4c 9c 10          	mov    0x10(%esp,%ebx,4),%ecx         
 8048e61:	83 f9 01             	cmp    $0x1,%ecx
 8048e64:	7e e4                	jle    8048e4a <phase_6+0x65>
 8048e66:	b8 01 00 00 00       	mov    $0x1,%eax
 8048e6b:	ba 3c c1 04 08       	mov    $0x804c13c,%edx 
 8048e70:	eb cc                	jmp    8048e3e <phase_6+0x59>
 8048e72:	8b 5c 24 28          	mov    0x28(%esp),%ebx
 8048e76:	8d 44 24 2c          	lea    0x2c(%esp),%eax
 8048e7a:	8d 74 24 40          	lea    0x40(%esp),%esi
 8048e7e:	89 d9                	mov    %ebx,%ecx
 8048e80:	8b 10                	mov    (%eax),%edx
 8048e82:	89 51 08             	mov    %edx,0x8(%ecx)
 8048e85:	83 c0 04             	add    $0x4,%eax
 8048e88:	39 f0                	cmp    %esi,%eax
 8048e8a:	74 04                	je     8048e90 <phase_6+0xab>
 8048e8c:	89 d1                	mov    %edx,%ecx
 8048e8e:	eb f0                	jmp    8048e80 <phase_6+0x9b>
 8048e90:	c7 42 08 00 00 00 00 	movl   $0x0,0x8(%edx)
 8048e97:	be 05 00 00 00       	mov    $0x5,%esi
 8048e9c:	8b 43 08             	mov    0x8(%ebx),%eax
 8048e9f:	8b 00                	mov    (%eax),%eax
 8048ea1:	39 03                	cmp    %eax,(%ebx)
 8048ea3:	7e 05                	jle    8048eaa <phase_6+0xc5>
 8048ea5:	e8 7b 02 00 00       	call   8049125 <explode_bomb>
 8048eaa:	8b 5b 08             	mov    0x8(%ebx),%ebx
 8048ead:	83 ee 01             	sub    $0x1,%esi
 8048eb0:	75 ea                	jne    8048e9c <phase_6+0xb7>
 8048eb2:	83 c4 44             	add    $0x44,%esp
 8048eb5:	5b                   	pop    %ebx
 8048eb6:	5e                   	pop    %esi
 8048eb7:	c3                   	ret    
...
08048eb8 <fun7>:
 ...
 8048ebc:	8b 54 24 20          	mov    0x20(%esp),%edx
 8048ec0:	8b 4c 24 24          	mov    0x24(%esp),%ecx
 8048ec4:	85 d2                	test   %edx,%edx
 8048ec6:	74 37                	je     8048eff <fun7+0x47>
 8048ec8:	8b 1a                	mov    (%edx),%ebx
 8048eca:	39 cb                	cmp    %ecx,%ebx
 8048ecc:	7e 13                	jle    8048ee1 <fun7+0x29>
 8048ece:	89 4c 24 04          	mov    %ecx,0x4(%esp)
 8048ed2:	8b 42 04             	mov    0x4(%edx),%eax
 8048ed5:	89 04 24             	mov    %eax,(%esp)
 8048ed8:	e8 db ff ff ff       	call   8048eb8 <fun7>
 8048edd:	01 c0                	add    %eax,%eax
 8048edf:	eb 23                	jmp    8048f04 <fun7+0x4c>
 8048ee1:	b8 00 00 00 00       	mov    $0x0,%eax
 8048ee6:	39 cb                	cmp    %ecx,%ebx
 8048ee8:	74 1a                	je     8048f04 <fun7+0x4c>
 8048eea:	89 4c 24 04          	mov    %ecx,0x4(%esp)
 8048eee:	8b 42 08             	mov    0x8(%edx),%eax
 8048ef1:	89 04 24             	mov    %eax,(%esp)
 8048ef4:	e8 bf ff ff ff       	call   8048eb8 <fun7>
 8048ef9:	8d 44 00 01          	lea    0x1(%eax,%eax,1),%eax
 8048efd:	eb 05                	jmp    8048f04 <fun7+0x4c>
 8048eff:	b8 ff ff ff ff       	mov    $0xffffffff,%eax
 8048f04:	83 c4 18             	add    $0x18,%esp
 8048f07:	5b                   	pop    %ebx
 8048f08:	c3                   	ret    
...
08048f09 <secret_phase>:
 ...
 8048f0d:	e8 8a 02 00 00       	call   804919c <read_line>
 8048f12:	c7 44 24 08 0a 00 00 	movl   $0xa,0x8(%esp)
 8048f19:	00 
 8048f1a:	c7 44 24 04 00 00 00 	movl   $0x0,0x4(%esp)
 8048f21:	00 
 8048f22:	89 04 24             	mov    %eax,(%esp)
 8048f25:	e8 a6 f9 ff ff       	call   80488d0 <strtol@plt>
 8048f2a:	89 c3                	mov    %eax,%ebx
 8048f2c:	8d 40 ff             	lea    -0x1(%eax),%eax
 8048f2f:	3d e8 03 00 00       	cmp    $0x3e8,%eax
 8048f34:	76 05                	jbe    8048f3b <secret_phase+0x32>
 8048f36:	e8 ea 01 00 00       	call   8049125 <explode_bomb>
 8048f3b:	89 5c 24 04          	mov    %ebx,0x4(%esp)
 8048f3f:	c7 04 24 88 c0 04 08 	movl   $0x804c088,(%esp)
 8048f46:	e8 6d ff ff ff       	call   8048eb8 <fun7>
 8048f4b:	83 f8 03             	cmp    $0x3,%eax
 8048f4e:	74 05                	je     8048f55 <secret_phase+0x4c>
 8048f50:	e8 d0 01 00 00       	call   8049125 <explode_bomb>
 8048f55:	c7 04 24 90 a1 04 08 	movl   $0x804a190,(%esp)
 8048f5c:	e8 8f f8 ff ff       	call   80487f0 <puts@plt>
 8048f61:	e8 30 03 00 00       	call   8049296 <phase_defused>
...
08048f70 <sig_handler>:
 ...
 8048f73:	c7 04 24 b8 a1 04 08 	movl   $0x804a1b8,(%esp)
 ...
 8048f8b:	c7 44 24 04 7a a2 04 	movl   $0x804a27a,0x4(%esp)
 8048f92:	08 
 ...
 8048fb8:	c7 04 24 82 a2 04 08 	movl   $0x804a282,(%esp)
...
08048fd0 <invalid_phase>:
 ...
 8048fdb:	c7 44 24 04 8a a2 04 	movl   $0x804a28a,0x4(%esp)
 8048fe2:	08 
...
08049125 <explode_bomb>:
 ...
 8049128:	c7 04 24 9b a2 04 08 	movl   $0x804a29b,(%esp)
 804912f:	e8 bc f6 ff ff       	call   80487f0 <puts@plt>
 8049134:	c7 04 24 a4 a2 04 08 	movl   $0x804a2a4,(%esp)
...
0804914c <read_six_numbers>:
 ...
 804917a:	c7 44 24 04 bb a2 04 	movl   $0x804a2bb,0x4(%esp)
 8049181:	08 
...
0804919c <read_line>:
 ...
 80491b8:	c7 04 24 cd a2 04 08 	movl   $0x804a2cd,(%esp)
 ...
 80491d0:	c7 04 24 eb a2 04 08 	movl   $0x804a2eb,(%esp)
 ...
 80491ff:	c7 04 24 cd a2 04 08 	movl   $0x804a2cd,(%esp)
 ...
 8049241:	c7 04 24 f6 a2 04 08 	movl   $0x804a2f6,(%esp)
 ...
 8049263:	be 11 a3 04 08       	mov    $0x804a311,%esi
...
08049296 <phase_defused>:
 ...
 80492c9:	c7 44 24 04 21 a3 04 	movl   $0x804a321,0x4(%esp)
 80492d0:	08 
 ...
 80492e2:	c7 44 24 04 2a a3 04 	movl   $0x804a32a,0x4(%esp)
 80492e9:	08 
 ...
 80492fa:	c7 04 24 f0 a1 04 08 	movl   $0x804a1f0,(%esp)
 8049301:	e8 ea f4 ff ff       	call   80487f0 <puts@plt>
 8049306:	c7 04 24 18 a2 04 08 	movl   $0x804a218,(%esp)
 804930d:	e8 de f4 ff ff       	call   80487f0 <puts@plt>
 8049312:	e8 f2 fb ff ff       	call   8048f09 <secret_phase>
 8049317:	c7 04 24 50 a2 04 08 	movl   $0x804a250,(%esp)
...
08049340 <sigalrm_handler>:
 ...
 804934a:	00 
 804934b:	c7 44 24 08 a0 a3 04 	movl   $0x804a3a0,0x8(%esp)
 8049352:	08 
...
08049483 <submitr>:
 ...
 80497e7:	c7 44 24 0c ac a4 04 	movl   $0x804a4ac,0xc(%esp)
 80497ee:	08 
 ...
 80499a0:	c7 44 24 04 b3 a4 04 	movl   $0x804a4b3,0x4(%esp)
 80499a7:	08 
 ...
 80499d2:	c7 44 24 0c c4 a3 04 	movl   $0x804a3c4,0xc(%esp)
 80499d9:	08 
 ...
 8049bc5:	c7 44 24 0c f4 a3 04 	movl   $0x804a3f4,0xc(%esp)
 8049bcc:	08 
...
08049cce <init_driver>:
 ...
 8049d90:	c7 04 24 c4 a4 04 08 	movl   $0x804a4c4,(%esp)
 ...
 8049e76:	c7 44 24 10 c4 a4 04 	movl   $0x804a4c4,0x10(%esp)
 8049e7d:	08 
 8049e7e:	c7 44 24 0c 84 a4 04 	movl   $0x804a484,0xc(%esp)
 8049e85:	08 
...
08049edc <driver_post>:
 ...
 8049ef7:	c7 44 24 04 d7 a4 04 	movl   $0x804a4d7,0x4(%esp)
 8049efe:	08 
 ...
 8049f30:	c7 44 24 10 ee a4 04 	movl   $0x804a4ee,0x10(%esp)
 8049f37:	08 
 8049f38:	89 44 24 0c          	mov    %eax,0xc(%esp)
 8049f3c:	c7 44 24 08 f2 a4 04 	movl   $0x804a4f2,0x8(%esp)
 8049f43:	08 
 8049f44:	c7 44 24 04 6e 3b 00 	movl   $0x3b6e,0x4(%esp)
 8049f4b:	00 
 8049f4c:	c7 04 24 c4 a4 04 08 	movl   $0x804a4c4,(%esp)
